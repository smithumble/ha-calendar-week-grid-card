...
const getMockEvents = () => {
  ...
  const probableGroupSlots =
    probableData['25']?.['dsos']?.['902']?.['groups']?.[GROUP_ID]?.['slots'];
...
(async () => {
  ...
  try {
    ...
    if (specificConfig && configsToProcess.length === 0) {
      console.error(
        `Config '${specificConfig}' not found. Use 'npm run list-configs' to see available configs.`,
      );
      process.exit(1);
    }
